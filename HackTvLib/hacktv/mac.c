...
/* Function to get the timezone offset in 30-minute intervals for Windows */
int get_timezone_offset() {
    TIME_ZONE_INFORMATION tz;
    GetTimeZoneInformation(&tz);
    return -tz.Bias / 30; // Bias is in minutes, divide by 30 to get 30-minute intervals
}

#else

/* Function to get the timezone offset in 30-minute intervals for Unix-like systems */
int get_timezone_offset(time_t timestamp) {
    struct tm tm;
    localtime_r(&timestamp, &tm);
    return tm.tm_gmtoff / 1800; // Offset in seconds divided by 1800 (30 minutes)
}

#endif

#if defined(__APPLE__) && defined(__MACH__)
/* macOS-specific function for gmtime_s */
int gmtime_s(struct tm *result, const time_t *timep) {
...
static void _update_udt(uint8_t udt[25], time_t timestamp)
{
...
#ifdef _WIN32
    /* Get the timezone offset for Windows */
    i = get_timezone_offset();
    if (i < 0) i = -i | (1 << 5);
...
#else
    /* Get the timezone offset for Unix-like systems */
    i = get_timezone_offset(timestamp);
    if (i < 0) i = -i | (1 << 5);
    ...
    mjd = 367 * (1900 + tm.tm_year)
          - (int) (7 * (1900 + tm.tm_year + (int) ((1 + tm.tm_mon + 9.0) / 12.0)) / 4.0)
          + (int) (275 * (1 + tm.tm_mon) / 9.0) + tm.tm_mday - 678987;
